 ...
 */
@Slf4j
public class MyNettyWebSocketChannelInboundHandlerAdapter2 extends SimpleChannelInboundHandler<Object> {
...
//		}
		...
		if (UtilString.isBlank(account)) {
			log.info("handleHttpRequest;非法连接:{};account:{}", req.uri(), account);
			ctx.disconnect();
			return;
		}
		...
		// nettyService.bind(account, header.getPlatform(), header.getDeviceId(),
		// header.getAppVersion(), ctx);

		nettyService.bind(account, "platform", "DeviceId", "AppVersion", ctx);

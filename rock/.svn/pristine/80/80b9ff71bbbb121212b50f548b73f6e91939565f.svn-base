...
public class RocketMqMapper implements IMqMapper {
	...
	public String sendMsg(String body, String topic, String tag) {
		...
		if (UtilString.isBlank(body)) {
			throw new RockException(RockEnum.PAEAM_ERR, "消息体不能为空");
		}
		if (UtilString.isBlank(topic)) {
			throw new RockException(RockEnum.PAEAM_ERR, "topic不能为空");
		}

		if (UtilString.isBlank(tag)) {
			throw new RockException(RockEnum.PAEAM_ERR, "tag不能为空");
		}
	...
	@Override
	public String sendDelayMsg(String body, IMqEnum mqEnum, Integer delayTime) {
